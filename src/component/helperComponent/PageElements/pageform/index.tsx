...
import { ColorSchemeName, Appearance, Image, View, Text, ScrollView, ScrollViewProps, ActivityIndicator, Keyboard, KeyboardAvoidingView, Platform, ScrollViewComponent } from 'react-native';
...
class PageForm extends React.PureComponent<TPageFormProps, TPageFormState>
{
    ...
    scrollViewRef = React.createRef<ScrollViewComponent | any>();
    _isMounted = false;
    ...
    renderFormField({ step, field, item, i, scrollToTop }: renderFormFieldType) {
        ...
        if (field.type.checkOption('Button')) {
            ...
            return (
                <>
                    {this.renderFields(field, fieldDisable, item, i, scrollToTop)}
                </>
            )
        }
    }
    ...
    renderFields(field: StepForm.FieldInfo, is_disabled: boolean, item: StepForm.FieldInfo[], i: number, scrollToTop: () => void) {
        const { step } = this.props;
        ...
        if (step.option_1 === 'SinglePageFormat') {
                                                            ...
                                                            singlePageItem.map((item, singlePageItemIndex) => {
                                                                return (
                                                                    <View key={'single_page_item_' + item.field + singlePageItemIndex}>
                                                                        {this.renderFormField({ step: this.props.step, field: item, item: singlePageItem, i: singlePageItemIndex, scrollToTop: scrollToTop })}
                                                                    </View>
                                                                )
                                                            })
                                                        }
            ...
            items.forEach((item: any) => {
                if (item.hasOwnProperty('key')) {
                    ...
                    const comp = this.renderFormField({ step: step, field: item, item: item, i: 0, scrollToTop: scrollToTop });
                    list.push({
                        key: 'item_' + keyNum++,
                        item: comp,
